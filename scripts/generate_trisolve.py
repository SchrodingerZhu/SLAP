...
SRC = """
module {
    func.func @trisolve(%b: memref<[:N:]xf32>, %L: memref<[:N:]x[:N:]xf32>, %x: memref<[:N:]xf32>) {
...
async def single_task(args, src, n):
    async with aiofiles.tempfile.NamedTemporaryFile("w") as f:
        ...
        proc_args = [
            ...
            f"{args.output_dir}/trisolve{n}.adj.json", "-d", f"{args.output_dir}/trisolve{n}.data.json"
        ]
        if args.average:
            proc_args.append("-A")
...
def main():
    parser = argparse.ArgumentParser(description='Generate trisolve')
