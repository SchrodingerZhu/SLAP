module {
  func.func @gemm(%A: memref<20x51xf32>, %B: memref<51x107xf32>, %C: memref<20x107xf32>, %alpha: f32, %beta: f32) {
    affine.for %i = 0 to 20 {
      affine.for %j = 0 to 107 {
        // Load the value from C and scale it by beta
        %c_val = affine.load %C[%i, %j] : memref<20x107xf32>
        %c_scaled = arith.mulf %c_val, %beta : f32
        
        // Initialize the accumulator
        %acc0 = arith.constant 0.0 : f32
        %sum = affine.for %k = 0 to 51 iter_args(%acc = %acc0) -> f32 {
          // Load values from A and B
          %a_val = affine.load %A[%i, %k] : memref<20x51xf32>
          %b_val = affine.load %B[%k, %j] : memref<51x107xf32>
        ...
        affine.store %final_val, %C[%i, %j] : memref<20x107xf32>
      }
    } { slap.extract }
    return
  }
